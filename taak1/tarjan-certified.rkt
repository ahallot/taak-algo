...
;           (vector-dag (make-vector nr-of-components '())))
;#t
;
;       ))
;         
         
  (list #f fw bw nr-of-components sc-components))
...
(define (call-all-on-graph g . text)
  (newline)
  (display (list (string-append "SCC-tarj-original  :  "  (car text))(test g)))
  (newline)
  (display (list (string-append "SCC-tarj-certifier  :  "  (car text))(scc-tarjan g)))
  (newline)
...
(define list-graph-test (list (cons paper-example "paper-example")
...
;(display (list "SCC-tarj    paper-example" (test             full-cycle)))
